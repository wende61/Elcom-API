    ...
    public class UserService : IUserService
    {
        ...
        public async Task<UserResponse> RegisterSupplier(SupplierRegistrationRequest request)
        {
            try
            {
                ...
                if (previousUser != null)
                    return new UserResponse { Message = Resources.RecordAlreadyExist, Status = OperationStatus.ERROR };
                var PreviousSupplier = _supplierRepository.Where(x => x.CompanyName == request.CompanyName && x.ContactEmail == request.ContactEmail && x.Website == request.Website).FirstOrDefault();
                ...
                try
                {
                    using (var uow = new AppUnitOfWork(_appTransaction.GetDbContext()))
                    {
                        ...
                        RepositoryBaseWork<User> userRepo = new RepositoryBaseWork<User>(uow);
        ...
        public Supplier GetSupplier(string userName)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(userName) || !string.IsNullOrWhiteSpace(userName))
                {
                    result = _userRepository.Where(x => x.Username == userName).Include(x=>x.Supplier).Select(x => x.Supplier).FirstOrDefault();
                    return result;
                }
                return result; 
            }
